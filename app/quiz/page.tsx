...
import { playBrowserQuestionSpeech, playHintSpeech, stopCharacterSpeech } from "@/lib/character-speech";
import { playQuestionIntroChime } from "../button-sound-provider";
...
export default function QuizPage() {
  ...
  const autoSpokenQuestionId=useRef<number|null>(null); const autoSpeechTimer=useRef<number|null>(null); const autoSpeechGeneration=useRef(0);
  ...
  useEffect(()=>{
    if(!question||autoSpokenQuestionId.current===question.sessionQuestionId)return;
    const generation=++autoSpeechGeneration.current;
    autoSpeechTimer.current=window.setTimeout(()=>{
      autoSpeechTimer.current=null;
      autoSpokenQuestionId.current=question.sessionQuestionId;
      void (async()=>{
        await playQuestionIntroChime();
        if(generation!==autoSpeechGeneration.current)return;
        try {
          await playBrowserQuestionSpeech({text:question.questionText,visemes:question.ttsVisemes});
        } catch {
          if(generation===autoSpeechGeneration.current)setMessage("자동 재생이 되지 않으면 다시 듣기를 눌러 주세요.");
        }
      })();
    },300);
    return ()=>{
      if(generation===autoSpeechGeneration.current)autoSpeechGeneration.current+=1;
      if(autoSpeechTimer.current!==null){window.clearTimeout(autoSpeechTimer.current);autoSpeechTimer.current=null;}
    };
  },[question]);
  useEffect(()=>()=>{
    autoSpeechGeneration.current+=1;
    if(autoSpeechTimer.current!==null)window.clearTimeout(autoSpeechTimer.current);
    stopCharacterSpeech();
  },[]);

  function cancelAutoSpeech(){
    autoSpeechGeneration.current+=1;
    if(autoSpeechTimer.current!==null){window.clearTimeout(autoSpeechTimer.current);autoSpeechTimer.current=null;}
  }
  ...
  function replay(){if(question){cancelAutoSpeech();playBrowserQuestionSpeech({text:question.questionText,visemes:question.ttsVisemes}).catch(error=>setMessage(`문제 음성 재생 실패 (${error instanceof Error?error.message:"UNKNOWN"})`));}}
  ...
  return <main className="page-shell"><div className="container">
    ...
    <section className="grid items-center gap-8 py-4 lg:grid-cols-[.75fr_1.25fr]"><div className="flex flex-col items-center"><div className="quiz-character-scene"><div className="quiz-character-scene-tint" aria-hidden="true"/><Buddy motion={status==="recording"?"listening":status==="processing"||hintThinking?"thinking":"idle"}/></div><div className="speech mt-5 w-full max-w-[410px] text-center"><p className="mb-2 text-sm font-extrabold text-(--accent)">{question?.questionTextKo??"문제를 불러오는 중"}</p><h1 className="speech-en">{question?.questionText??"…"}</h1><button type="button" onClick={replay} disabled={!question} className="btn btn-ghost mt-4 text-sm disabled:opacity-40">🔊 다시 듣기</button></div></div>
      <div className="card quiz-media-card p-6 md:p-9"><div className="quiz-type-row"><span className="quiz-type-label">문제 유형</span><span className="quiz-type-badge"><span aria-hidden="true">{question?.type==="PICTURE_DESCRIPTION"?"▧":"●"}</span>{question?.type==="PICTURE_DESCRIPTION"?"그림 보고 말하기":"영어로 말하기"}</span></div><div className="grid min-h-[330px] place-items-center rounded-[28px] bg-gradient-to-br from-[#fffdf5] to-[#fff1e5] p-4 md:p-6">{question?.imageUrl?<div role="img" aria-label={question.questionTextKo} className="aspect-[4/3] w-full max-w-[640px] overflow-hidden rounded-[28px] border-4 border-white bg-contain bg-center bg-no-repeat shadow-[0_10px_30px_rgba(75,75,75,.14)] ring-2 ring-[#eadfce]" style={{backgroundImage:`url("${question.imageUrl.replaceAll('"','%22')}")`}}/>:<span className="text-[110px]" aria-hidden>💬</span>}</div></div>
    </section>
