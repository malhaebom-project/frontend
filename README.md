...
한국어 AI 피드백에는 Azure Speech의 한국어 HD 음성을 사용할 수 있습니다. Azure Portal에서 Speech 리소스를 만든 뒤 서버 전용 환경변수를 설정합니다. `AZURE_SPEECH_KEY`에는 `NEXT_PUBLIC_` 접두사를 붙이지 않습니다.

```env
AZURE_SPEECH_KEY=발급받은_키
AZURE_SPEECH_REGION=koreacentral
AZURE_KOREAN_FEEDBACK_VOICE=ko-KR-SunHi:DragonHDLatestNeural
```

설정되면 피드백 화면은 Azure에서 생성한 MP3와 비짐 타임라인을 함께 사용합니다. 설정되지 않았거나 Azure 요청에 실패하면 브라우저의 한국어 음성으로 자동 대체됩니다. 영어 문제 음성에는 이 설정이 적용되지 않습니다.
