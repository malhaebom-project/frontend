...
import { childWithSubject, parentPraiseMessage, randomParentPraiseIndex } from "@/lib/parent-praise";

export default function RecordsPage() {
  const [initialChild]=useState<Child|null>(()=>learningState.child()); const [children,setChildren]=useState<Child[]>([]); const [child,setChild]=useState<Child|null>(initialChild); const [history,setHistory]=useState<LearningHistory|null>(null); const [stats,setStats]=useState<Statistics|null>(null); const [wrong,setWrong]=useState<WrongAnswer[]>([]); const [preview,setPreview]=useState(false); const [error,setError]=useState(""); const [loading,setLoading]=useState(true); const [apiReady,setApiReady]=useState(false); const [page,setPage]=useState(0); const [startDate,setStartDate]=useState(""); const [endDate,setEndDate]=useState(""); const [appliedStartDate,setAppliedStartDate]=useState(""); const [appliedEndDate,setAppliedEndDate]=useState(""); const [praiseIndex,setPraiseIndex]=useState(0); const requestGeneration=useRef(0);
  useEffect(()=>{
    ...
    async function prepareRecords(){
      try {
        if(!isAuthenticated()){
          ...
          saveSessionValue("child",data.child);setChildren([data.child]);setChild(data.child);setHistory(data.history);setStats(data.statistics);setWrong(data.wrongAnswers);setPraiseIndex(randomParentPraiseIndex());setPreview(true);
          return;
        }
  ...
  useEffect(()=>{
    ...
    ]).then(([nextHistory,nextStats,nextWrong])=>{if(active&&generation===requestGeneration.current){setHistory(nextHistory);setStats(nextStats);setWrong(nextWrong);setPraiseIndex(randomParentPraiseIndex());}}).catch(e=>{if(active&&generation===requestGeneration.current)setError(errorMessage(e));}).finally(()=>{if(active&&generation===requestGeneration.current)setLoading(false);});
    return()=>{active=false;};
  ...
  return <main className="min-h-screen bg-(--app-bg)"><div className="border-b border-(--app-bg-border) bg-white"><header className="topbar container"><Brand/><div className="flex items-center gap-3"><MiniProfile/><Link href="/" className="btn btn-ghost">아이 화면으로</Link></div></header></div>
    <div className="container py-12"><div className="flex flex-wrap items-end justify-between gap-5"><div><p className="eyebrow">Parent dashboard</p><h1 className="title mt-2">학습 기록</h1><p className="subtitle mb-0 mt-2">{child?.nickname??"아이"}의 영어 자신감이 얼마나 자랐는지 확인해 보세요.</p></div><div className="flex flex-wrap items-center gap-3">{preview&&<span className="pill">체험용 샘플 기록</span>}{children.length>0&&<label className="flex h-12 items-center gap-2 rounded-xl border border-(--accent-border) bg-white px-3 text-sm font-extrabold text-(--muted-2)">아이 선택<select aria-label="학습 기록을 확인할 어린이 선택" value={child?.childId??""} disabled={loading} onChange={event=>selectChild(Number(event.target.value))} className="bg-transparent font-black text-(--navy) outline-none disabled:cursor-wait">{children.map(profile=><option key={profile.childId} value={profile.childId}>{profile.nickname}</option>)}</select></label>}<Link href="/profiles" className="btn btn-ghost">프로필 관리</Link></div></div>
      ...
      {stats&&<ParentGuide childName={child?.nickname??"아이"} totalSessionCount={stats.totalSessionCount} allStarsEarned={Boolean(history?.content[0]&&history.content[0].questionCount>0&&history.content[0].correctCount===history.content[0].questionCount)} praiseIndex={praiseIndex}/>}
...
function ParentGuide({childName,totalSessionCount,allStarsEarned,praiseIndex}:{childName:string;totalSessionCount:number;allStarsEarned:boolean;praiseIndex:number}) {
  const guide=totalSessionCount===0
    ? {icon:"🌱",eyebrow:"첫 학습 안내",title:"첫 학습을 함께 시작해 보세요!",message:`${childWithSubject(childName)} 부담 없이 첫 문장을 말할 수 있도록 옆에서 응원해 주세요. 짧게 한 문제부터 시작해도 충분해요.`,theme:"border-[#cfe4ff] from-[#f2f8ff] to-[#f6fffb] shadow-[0_6px_0_#dceaf7]",accent:"text-[#3b78b8]"}
    : allStarsEarned
      ? {icon:"🌟",eyebrow:"오늘의 별 다섯 개 칭찬",title:"모든 별을 모았어요!",message:parentPraiseMessage("allStars",praiseIndex,childName),theme:"border-[#cdeebc] from-[#f4ffec] to-[#fffbe8] shadow-[0_6px_0_#d9edc9]",accent:"text-(--green-dark)"}
      : {icon:"💪",eyebrow:"도전 과정 응원",title:"노력한 과정을 응원해 주세요!",message:parentPraiseMessage("progress",praiseIndex,childName),theme:"border-[#f4df9a] from-[#fffbea] to-[#fff5e9] shadow-[0_6px_0_#f2e7bd]",accent:"text-[#9a6a13]"};
