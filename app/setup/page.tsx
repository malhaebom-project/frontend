...
import { resetRewardLedger } from "@/lib/rewards";
...
export default function SetupPage() {
  ...
  async function start(){
    ...
    try {const session=await api.createSession({childId:child.childId,topicId,difficulty,questionTypes:types,questionCount:count});resetRewardLedger(session);saveSessionValue("session",session);router.push("/quiz");}
  ...
  return <main className="page-shell"><div className="container">
    ...
    <section className="mx-auto max-w-5xl py-10 text-center"><p className="eyebrow">Today&apos;s adventure</p><h1 className="title mt-3">오늘 무엇을 배워볼까요?</h1><p className="subtitle mt-3">게임을 고르듯 즐겁게 선택해 보세요.</p>
      ...
      <div className="mt-10 grid gap-5 md:grid-cols-3">{topics.map(item=>{const [emoji,tone]=topicArt[item.code]??["💬","soft-blue"];const selected=topicId===item.topicId;return <button key={item.topicId} onClick={()=>setTopicId(item.topicId)} className={`card relative min-h-[210px] cursor-pointer p-6 transition hover:-translate-y-2 ${selected?"border-2 border-[#58cc02]":""}`}>{selected&&<span className="absolute right-4 top-4 grid h-8 w-8 place-items-center rounded-full bg-[#58cc02] text-white">✓</span>}<span className={`mx-auto grid h-24 w-24 place-items-center rounded-[30px] ${tone} text-5xl`}>{emoji}</span><strong className="mt-4 block text-2xl">{item.name}</strong></button>})}</div>
...
function Choice({label,values,selected,onSelect}:{label:string;values:string[][];selected:string;onSelect:(v:string)=>void}){return <fieldset><legend className="mb-3 font-extrabold">{label}</legend><div className="grid grid-cols-2 gap-2">{values.map(([value,text])=><button type="button" key={value} onClick={()=>onSelect(value)} className={`btn px-3 ${selected===value?"bg-[#f2ffe9] text-[#46a302] ring-2 ring-[#58cc02]":"bg-[#f4f7fb] text-[#777]"}`}>{selected===value&&"✓ "}{text}</button>)}</div></fieldset>}
